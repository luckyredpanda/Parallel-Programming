...
image_gpu::image_gpu(int w, int h) : width(w), height(h)
{
	...
	//unsigned int *data;
	cudaMalloc((void**)&data, w*h*sizeof(int));
...
void image_cpu::upload(image_gpu &dst) const
{
	...
	cudaMemcpy( (void*)dst.data, (void*)data, sizeof(int)* width * height, cudaMemcpyHostToDevice);
	//cudaMemcpy2D((void*)dst.data,
	//			 dst.pitch,
	//			 (void*)data,
	//			 width * sizeof(int),
	//			 width * sizeof(int),
...
void image_cpu::download(const image_gpu &src)
{
	...
	cudaMemcpy( (void*)data, (void*)src.data, sizeof(int)* width * height, cudaMemcpyDeviceToHost);
	//cudaMemcpy2D((void*)data,
	//			 width * sizeof(int),
	//			 (void*)src.data,
	//			 src.pitch,
